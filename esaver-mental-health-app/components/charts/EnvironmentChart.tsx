...
export default function EnvironmentChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const chartData = json.temperature.map((temp: number, i: number) => ({
          time: `${i}h`,
          temperature: temp,
          humidity: json.humidity[i],
          pressure: json.pressure[i],
        }))
        setData(chartData)
      } catch (error) {
  ...
  return (
    <ChartCard title="Environmental Context" loading={loading}>
